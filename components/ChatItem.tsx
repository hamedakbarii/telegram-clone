    ...
    <div className="border-b border-transparent">
      <button className="w-full p-3 flex items-center gap-3 hover:bg-[#151515] dark:hover:bg-[#151515] transition-colors cursor-pointer">
        ...
        <div className="flex-1 min-w-0 text-left">
          <div className="flex justify-start items-center min-w-0">
            ...
            <div className="flex mr-0.5">
              <span className="flex items-center text-xs text-gray-100 whitespace-nowrap">
                {chat.messageStatus === "read" && (<BiCheckDouble fontSize={20} />)}
                {chat.messageStatus === "succeeded" && (<FaCheck />)}
                <p className={`${chat.messageStatus === "succeeded" && "ml-1"}`}>
          ...
          <div className="flex justify-start items-center min-w-0">
            <p className={`text-sm text-gray-500 truncate text-left grow pr-1 ${chat.isArchive && "font-bold"}`}>
              {searchQuery ? highlightText(chat.lastMessage, searchQuery) : chat.lastMessage}
            </p>
            {chat.unreadCount > 0 && (
              <span className={`text-xs rounded-full w-5 h-5 flex items-center justify-center p-3 ${
